...
hold on
textscatter3(ModelParams.R(3)*Elec_Pos(1,:),ModelParams.R(3)*Elec_Pos(2,:),ModelParams.R(3)*Elec_Pos(3,:),Elec_name);
%% section c
clc;
x=-1 ;y=3;z=7.2;
dipole_index=1295;
...
hold on
plot3( [ vector_normalize(1,1)+LocMat(1,dipole_index),LocMat(1,dipole_index) ] ,[ vector_normalize(2,1)+LocMat(2,dipole_index),LocMat(2,dipole_index)] , [vector_normalize(3,1)+LocMat(3,dipole_index),LocMat(3,dipole_index)] ,'->')
xlabel('x')
ylabel('y')
zlabel('z')
...
scatter3(LocMat(1,dipole_index),LocMat(2,dipole_index),LocMat(3,dipole_index),'g','*');
hold on
plot3( [ ModelParams.R(1)*vector_normalize(1,1)+LocMat(1,dipole_index),LocMat(1,dipole_index) ] ,[ ModelParams.R(1)*vector_normalize(2,1)+LocMat(2,dipole_index),LocMat(2,dipole_index)] , [ModelParams.R(1)*vector_normalize(3,1)+LocMat(3,dipole_index),LocMat(3,dipole_index)] ,'->')
xlabel('x')
ylabel('y')
zlabel('z')
...
[X,Y,T,AUC,OPTROCPT] = perfcurve(labels,Q_MNE_hat,1) ;
figure()
plot( X , Y)
title("  MNE")
hold on
...
[X,Y,T,AUC,OPTROCPT] = perfcurve(labels,Q_WMNE_hat,1) ;
figure()
plot( X , Y)
title(" WMNE")
hold on
...
labels = zeros(1,1317);
labels(index_dipoles) = 1;
[X,Y,T,AUC,OPTROCPT] = perfcurve(labels,Q_Loreta_hat,1) ;
figure()
plot( X , Y)
title(" Loreta ")
hold on
